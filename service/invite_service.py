...
class InviteService:
    ...
    def register_through_invite(
        ...
        auth = AuthService()
        raw_verify, _ = auth.create_email_verification_token(user)
        db.session.commit()

        email_sent = auth._send_verification_email(user, raw_verify)
        result = {
        ...
        if not email_sent and current_app.config.get("DEBUG"):
            result["dev_verification_token"] = raw_verify
        return result
