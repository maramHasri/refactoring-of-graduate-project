...
class InviteService:
        ...
        *,
        ...
        if not can_invite_with_role(
            ...
            if inviter_role == MembershipRole.TEACHER.value:
                raise ForbiddenError(Messages.TEACHERS_CAN_ONLY_INVITE_STUDENTS)
            if inviter_role == MembershipRole.STUDENT.value:
                raise ForbiddenError(Messages.STUDENTS_CANNOT_SEND_INVITATIONS)
            raise ForbiddenError(Messages.INSUFFICIENT_PERMISSIONS_TO_INVITE_THIS_ROLE)

        if workspace.kind == WorkspaceKind.SOLO.value:
            if assigned_role != MembershipRole.STUDENT.value:
                raise ForbiddenError(Messages.SOLO_WORKSPACES_CAN_ONLY_INVITE_STUDENTS_OWNER_IS_THE_TEACHER)
        ...
        if assigned_role not in (
            ...
            raise ValidationError(Messages.INVALID_ROLE_FOR_INVITATION)
        ...
        if existing:
            ...
            raise ConflictError(Messages.PENDING_INVITE_ALREADY_EXISTS_FOR_THIS_EMAIL)
    ...
    def register_through_invite(
        ...
        if self.users.find_by_email(invite.email):
            raise ConflictError(Messages.ACCOUNT_ALREADY_EXISTS_PLEASE_LOGIN_FIRST)
        ...
        result = {
            "message": Messages.REGISTRATION_SUCCESSFUL_CHECK_YOUR_EMAIL_FOR_THE_VERIFICATION_CODE,
            "user_id": user.id,
            "workspace_id": invite.workspace_id,
    ...
    def accept_invite(self, raw_token: str, user_id: int) -> Membership:
        ...
        if not user or user.email.lower() != invite.email.lower():
            raise ForbiddenError(Messages.INVITE_EMAIL_DOES_NOT_MATCH_AUTHENTICATED_USER)
    ...
    def reject_invite(self, raw_token: str) -> None:
        ...
        if invite.status != InviteStatus.PENDING.value:
            raise ValidationError(Messages.INVITATION_IS_NO_LONGER_VALID)
        if invite.revoked_at is not None:
            raise ValidationError(Messages.INVITATION_HAS_BEEN_REVOKED)
    ...
    def revoke_pending_invite(
        ...
        if not invite:
            raise NotFoundError(Messages.PENDING_INVITATION_NOT_FOUND)
    ...
    def _get_valid_pending_invite(self, raw_token: str, *, mutate: bool) -> WorkspaceInvite:
        ...
        if invite.status == InviteStatus.REJECTED.value:
            raise ValidationError(Messages.INVITATION_WAS_REJECTED)
        if invite.status == InviteStatus.ACCEPTED.value:
            raise ValidationError(Messages.INVITATION_HAS_ALREADY_BEEN_ACCEPTED)
        if invite.status != InviteStatus.PENDING.value:
            raise ValidationError(Messages.INVITATION_IS_NO_LONGER_VALID)
        if invite.expires_at < now:
            if mutate:
                invite.status = InviteStatus.EXPIRED.value
                db.session.commit()
            raise ValidationError(Messages.INVITATION_HAS_EXPIRED)
        return invite
